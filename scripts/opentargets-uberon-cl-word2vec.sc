...
//          ner,
  ...
  def loadTissues(path: String)(implicit sparkSession: SparkSession): DataFrame = {
    sparkSession.read
      .options(Map(
        "sep" -> "\t",
        "inferSchema" -> "true",
      ...
      .csv(path)
      .withColumnRenamed("id", "traitId")
      .withColumnRenamed("infoline", "traitName")
      .groupBy(col("traitName"))
      .agg(collect_list(col("traitId")).as("traitId"))
  }

  def apply(tissues: String, ontology: String, output: String) = {
    import SparkSessionWrapper._
    import spark.implicits._
    implicit val ss: SparkSession = spark

    logger.info("load required datasets from ETL parquet format")
    val diseases = spark.read.json(ontology)

    val traits = loadTissues(tissues)
    val pipeline = generatePipeline("text", columnsToInclude)

    val D = diseases
      .selectExpr("id as efoId", "label as efoName", "synonyms.*")
...
@main
def main(tissues: String, ontology: String, output: String): Unit =
  ETL(tissues, ontology, output)
