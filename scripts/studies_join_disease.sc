...
@main
def main(step: String = ""): Unit = {
  val spark = SparkSessionBuilder()

  // copy from here latest
  // gs://open-targets-data-releases/20.06/output/parquet/efo_parquet
  val diseases = spark.read.parquet("efo_parquet")

  // copy from here latest release data
  // gs://genetics-portal-output/20022712/lut/study-index
  val studies = spark.read.json("study-index")

  /*
    get all ancestors from disease and then explode to get the
   */
    val expanded = diseases
      .withColumn("id", substring_index(col("code"), "/", -1))
      .withColumn(
      "ancestors",
        array_except(
