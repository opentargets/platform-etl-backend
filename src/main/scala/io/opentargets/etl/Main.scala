...
import io.opentargets.etl.backend.genetics.{
  DiseaseVariantGene,
  VariantDisease,
  VariantDiseaseColoc,
  VariantGene
  ...
  def applySingleStep(step: String)(implicit context: ETLSessionContext): Unit = {
    step.toLowerCase match {
      ...
      case "variantdiseasecoloc" =>
        ...
        VariantDiseaseColoc()
      case "diseasevariantgene" =>
        logger.info("run step disease-variant-gene (genetics")
        DiseaseVariantGene()
//      case "scoreddatasets" =>
//        logger.info("run step scoreddatasets (genetics")
//        ScoredDatasets()
  ...
  def stepsWithExistingOuputs(implicit ctx: ETLSessionContext): Set[String] = {
    lazy val outputPaths: Map[String, String] = Map(
      ...
      "diseaseVariantGene" -> ctx.configuration.diseaseVariantGene.outputs.diseaseVariantGene.path
    )
