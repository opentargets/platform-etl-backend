...
object Molecule extends LazyLogging {
  ...
  /** @param chemblMoleculeRaw
    *   of raw molecule inputs
    * @param drugbank
    *   drugbank lookup table of chembl_id -> drugbank_id
    * @return
  ...
  /** Method to group synonyms into sorted sets of trade names and others synonyms.
    ...
    *   df prepared with moleculePreprocess method
    * @return
    *   dataframe of `id: String, tradeName: Set[String], synonym: Set[String]`
    */
  ...
  /** Group all child molecules by chembl_id
    * @param preProcessedMolecules
    *   df produced by this.moleculePreprocess
    * @return
    *   dataframe of two columns `id`: Str, `child_chembl_ids`: Array[Str]
    */
  ...
  /** Method to group cross references for each molecule id. Source ids are grouped according to
    * source.
    *
    * @param preProcessedMolecules
    *   df prepared with moleculePreprocess methods
    * @return
    *   dataframe of `id: String, source: Map(src: String -> ids: Array[String])`
    */
  ...
  /** @param preProcessedMolecules
    *   generated by `molecule preprocess`
    * @return
    *   dataframe of: id, map(str, array[str])
    */
  ...
  /** Helper method to link singleton references to a ChEMBL id and return a cross reference map.
    *
    * Although the referenceIdColumn must be a singleton (str, int, long, etc) the method returns a
    * map with an array of values so that types are compatible with sources that have several
    * references.
    *
    * @param preProcessedMolecules
    *   dataframe prepared by `moleculePreprocess` method
    * @param referenceIdColumn
    *   column in preProcessed molecules to use as reference ids. Must only be single values.
    * @param source
    *   name of source used as key in returned map
    * @return
    *   chembl_id, Map(source -> Array[ref])
    */
  ...
  /** Helper method which takes in two dataframes of cross references and returns a single dataframe
    * with the cross reference maps merged.
    *
    * @param ref1
    *   dataframe with columns `id` and `xref`
    * @param ref2
    *   dataframe with columns `id` and `xref`
    * @return
    */
  ...
  /** Helper function to created maps where references are grouped by source.
    *
    * @param refSrcAndId
    *   sequence on inner pairs (src, id). Need to use seq rather than tuple for Spark to handle it
    *   properly.
    * @return
