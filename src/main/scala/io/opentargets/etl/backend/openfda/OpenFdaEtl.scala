package io.opentargets.etl.backend.openfda

import com.typesafe.scalalogging.LazyLogging
import io.opentargets.etl.backend.ETLSessionContext
import io.opentargets.etl.backend.openfda.stage.StratifiedSampling
import io.opentargets.etl.backend.openfda.utils.Loaders
...
object OpenFdaEtl extends LazyLogging {
  ...
  def apply(implicit etLSessionContext: ETLSessionContext): DataFrame = {
    ...
    val resultsWithOptionalMeddra =
      etLSessionContext.configuration.openfda.fdaInputs.meddraRelease match {
        case Some(value) =>
    ...
    if (etLSessionContext.configuration.openfda.sampling.enabled) {
