...
object TargetAttachMeddra {
  def apply(fdaData: DataFrame,
    ...
    val meddraPreferred = meddraPreferredTermsData
      ...
      .select(context.configuration.openfda.meddraPreferredTermsCols.zipWithIndex.map(i =>
        col("_c0").getItem(i._2).as(s"${i._1}")): _*)
    ...
    val meddraLowLevel = meddraLowLevelTermsData
      ...
      .withColumn("_c0", regexp_replace(col("_c0"), "\\$$", ""))
      .withColumn("_c0", split(col("_c0"), ","))
      .select(context.configuration.openfda.meddraLowLevelTermsCols.zipWithIndex.map(i =>
        col("_c0").getItem(i._2).as(s"${i._1}")): _*)
    ...
    val fdaMeddraPreferred = fdaData.join(dfMeddraPrefferedTerms, fdaData("reaction_reactionmeddrapt") === dfMeddraPrefferedTerms("pt_name"), "left_outer")
    // Add Meddra Low Level Terms
    val fdaMeddraPreferredAndLowLevel = fdaMeddraPreferred.join(dfMeddraLowLevelTerms,
