...
object TargetEngineTest {
  def createQuerySetDf(implicit context: ETLSessionContext): DataFrame = {
    ...
    context.sparkSession.createDataFrame(context.sparkSession.sparkContext.parallelize(rows), schema)
  }
  ...
  def createMouseDf(implicit context: ETLSessionContext): DataFrame = {
    ...
    val modelPhenotypeClassSchema = StructType(StructField("id", StringType, nullable = false) :: StructField("label", StringType, nullable = false) :: Nil)
  ...
  def createMousePhenoScoreDf(implicit context: ETLSessionContext): DataFrame = {
    val rows = Seq(Row("MP:0003631", 0.75), Row("MP:0005386", 0.75), Row("MP:0005377", 0.5), Row("MP:0005397", 0.5), Row("MP:0005387", 0.5))
    val schema = StructType(StructField("id", StringType, nullable = false) :: StructField("score", DoubleType, nullable=false):: Nil)
    context.sparkSession.createDataFrame(context.sparkSession.sparkContext.parallelize(rows), schema)
  }
}
...
class TargetEngineTest extends EtlSparkUnitTest {
  ...
  "Function mousemodQuery" should "return  dataframe" in {
    ...
    val expectedColumns = Set("targetId", "target_id_", "score", "harmonicSum", "maxHarmonicSum", "maximum", "scaledHarmonicSum", "negScaledHarmonicSum")
